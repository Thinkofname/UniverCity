...
impl Renderer {
    ...
    pub fn new(
        ...
        let animated_info =
            animated_model::Info::new(&log, &asset_manager, &mut pipeline.context());
    ...
    fn build_pipeline(
        ...
        for (i, sample) in KERNEL_SAMPLE
            .iter()
            .cloned()
            .enumerate()
            .take(ssao_level as usize)
        {
        ...
        noise_tex.image_2d_any(
            gl::TextureTarget::Texture2D,
            0,
            4,
            4,
            gl::TextureFormat::Rgb16F,
            gl::TextureFormat::Rgb,
            gl::Type::Float,
            Some(NOISE),
        );
        noise_tex.set_parameter::<gl::TextureMinFilter>(
            gl::TextureTarget::Texture2D,
            gl::TextureFilter::Nearest,
        );
        noise_tex.set_parameter::<gl::TextureMagFilter>(
            gl::TextureTarget::Texture2D,
            gl::TextureFilter::Nearest,
        );
        noise_tex.set_parameter::<gl::TextureWrapS>(
            gl::TextureTarget::Texture2D,
            gl::TextureWrap::Repeat,
        );
        noise_tex.set_parameter::<gl::TextureWrapT>(
            gl::TextureTarget::Texture2D,
            gl::TextureWrap::Repeat,
        );

        let fullscreen_attribs = &[("attrib_position", 0)];

        Pipeline::<PassFlag>::new(log, asset_manager.clone(), render_scale)
            // UI Shaders
            .program("ui/clip", |_, p| {
                p.vertex("ui/clip_vert")
                    .fragment("ui/clip_frag")
                    .attribute_binds(&[("attrib_position", 0)])
            })
            .program("ui/box", |_, p| {
                p.vertex("ui/box_vert")
                    .fragment("ui/box_frag")
                    .attribute_binds(&[("attrib_position", 0), ("attrib_color", 1)])
            })
            .program("ui/box_shadow", |_, p| {
                p.vertex("ui/box_shadow_vert")
                    .fragment("ui/box_shadow_frag")
                    .attribute_binds(&[("attrib_position", 0), ("attrib_color", 1)])
            })
            .program("ui/box_shadow_inner", |_, p| {
                p.vertex("ui/box_shadow_vert")
                    .fragment("ui/box_shadow_inner_frag")
                    .attribute_binds(&[("attrib_position", 0), ("attrib_color", 1)])
            })
            .program("ui/image", |_, p| {
                p.vertex("ui/image_vert")
            ...
            .program("ui/border_image", |_, p| {
                p.vertex("ui/border_image_vert")
            ...
            .program("ui/border_solid", |_, p| {
                p.vertex("ui/border_solid_vert")
            ...
            // Game shaders
            .program("merge", |_, p| {
                p.vertex("fullscreen_vert")
                    .fragment("merge")
                    .when(ssao_enabled, |p| p.fragment_defines(vec!["ssao_enabled"]))
                    .when(shadows_enabled, |p| {
                        p.fragment_defines(vec!["shadows_enabled"])
                    })
                    .attribute_binds(fullscreen_attribs)
            })
            .program("pause_effect", |_, p| {
                p.vertex("fullscreen_vert")
                    .fragment("pause_effect")
                    .attribute_binds(fullscreen_attribs)
            })
            .program("focused_effect", |_, p| {
                p.vertex("fullscreen_vert")
                    .fragment("focused_effect")
                    .attribute_binds(fullscreen_attribs)
            })
            .program("scale", |_, p| {
                p.enabled(render_scaling)
            ...
            .program("fxaa", |_, p| {
                p.enabled(fxaa_enabled)
                    .vertex("fullscreen_vert")
                    .fragment("fxaa")
                    .attribute_binds(fullscreen_attribs)
            })
            .program("ssao", |_, p| {
                p.enabled(ssao_enabled)
                    .vertex("fullscreen_vert")
                    .fragment("ssao")
                    .fragment_defines(vec![format!("kernel_size {}", ssao_level)])
                    .attribute_binds(fullscreen_attribs)
            })
            .program("blur_ssao_horz", |_, p| {
                p.enabled(ssao_enabled)
                    .vertex("fullscreen_vert")
                    .fragment("blur_ssao")
            ...
            .program("blur_color_horz", |_, p| {
                p.vertex("fullscreen_vert")
                    .fragment("blur_color")
                    .fragment_defines(vec!["HORZ"])
                    .attribute_binds(fullscreen_attribs)
            })
            .program("blur_color_vert", |_, p| {
                p.vertex("fullscreen_vert")
                    .fragment("blur_color")
                    .fragment_defines(vec!["VERT"])
                    .attribute_binds(fullscreen_attribs)
            ...
            .program("cursor", |_, p| {
                p.vertex("cursor_vert")
                    .fragment("cursor_frag")
                    .attribute_binds(&[("attrib_position", 0)])
            })
            .program("placement", |_, p| {
                p.vertex("placement_vert")
                    ...
                    .attribute_binds(&[("attrib_position", 0), ("attrib_lookup", 1)])
            })
            .program("icon", |_, p| {
                p.vertex("icon_vert")
                    .fragment("icon_frag")
            ...
            .program("terrain_selection", |c, p| {
                ...
                p.vertex("terrain_vert")
                    ...
                    .attribute_binds(&[
                        (
                            "attrib_position",
                            assume!(log, base.attribute("attrib_position")).index(),
                        ),
                        (
                            "attrib_normal",
                            assume!(log, base.attribute("attrib_normal")).index(),
                        ),
                        (
                            "attrib_texture",
                            assume!(log, base.attribute("attrib_texture")).index(),
                        ),
                    ])
            })
            .program("terrain_shadow", |c, p| {
                let base = c.program("terrain");
                p.enabled(shadows_enabled)
                    .vertex("terrain_vert")
            ...
            .program("static_water", |c, p| {
                ...
                p.vertex("static_vert")
                    ...
                    .attribute_binds(&[
                        ...
                        (
                            "attrib_uv",
                            assume!(log, base.attribute("attrib_uv")).index(),
                        ),
                        (
                            "attrib_matrix",
                            assume!(log, base.attribute("attrib_matrix")).index(),
                        ),
                        (
                            "attrib_tint",
                            assume!(log, base.attribute("attrib_tint")).index(),
                        ),
                        (
                            "attrib_highlight",
                            assume!(log, base.attribute("attrib_highlight")).index(),
                        ),
                    ])
            })
            ...
            .program("animated", |_, p| {
                p.vertex("animated_vert")
            ...
            .program("animated_shadow", |c, p| {
                ...
                p.enabled(shadows_enabled)
                    ...
                    .attribute_binds(&[
                        ...
                        (
                            "attrib_bones",
                            assume!(log, base.attribute("attrib_bones")).index(),
                        ),
                        (
                            "attrib_bone_weights",
                            assume!(log, base.attribute("attrib_bone_weights")).index(),
                        ),
                        (
                            "attrib_matrix",
                            assume!(log, base.attribute("attrib_matrix")).index(),
                        ...
                        (
                            "attrib_bone_offset",
                            assume!(log, base.attribute("attrib_bone_offset")).index(),
                        ),
                        (
                            "attrib_tint_offset",
                            assume!(log, base.attribute("attrib_tint_offset")).index(),
                        ),
                    ])
            })
            ...
            .pass("blur_highlight_horz", |_, p| {
                p.attachment("blur_color", |a| {
                    a.ty(pipeline::Type::U8)
                        .components(pipeline::Components::RGB)
                        .target(pipeline::AttachTarget::Color)
                        .linear()
            ...
            .pass("blur_highlight_vert", |_, p| {
                ...
                .fullscreen(|f| {
                    f.shader("blur_color_vert").input(
                        "g_input",
                        "blur_highlight_horz",
                        "blur_color",
                    )
                ...
                p.enabled(ssao_enabled)
                    ...
                    .fullscreen(|f| {
                        f.shader("ssao")
                            ...
                            .pre(move |ctx| {
                                let projection = *ctx
                                    .var::<Matrix4<f32>>("projection")
                                    .expect("Missing projection matrix");
                                ...
                                gl::active_texture(0);
                                p.uniform("samples[0]")
                                    .map(|v| v.set_vec3_array(&ssao_kernel));
            ...
            .pass("blur_ssao_horz", |_, p| {
                p.enabled(ssao_enabled)
                    .scale(2)
                    .attachment("blur_ssao", |a| {
                        a.ty(pipeline::Type::U8)
                            .components(pipeline::Components::R)
                            .target(pipeline::AttachTarget::Color)
                            .scale(2)
                    ...
                    .fullscreen(|f| f.shader("blur_ssao_horz").input("g_ssao", "ssao", "ssao"))
                    .clear_flags(gl::BufferBit::COLOR)
                    .flag(PassFlag::empty())
            })
            .pass("blur_ssao_vert", |_, p| {
                p.enabled(ssao_enabled)
                    .scale(2)
                    .attachment("blur_ssao", |a| {
                        a.ty(pipeline::Type::U8)
                ...
                p.final_color()
                    .fullscreen(|f| {
                        f.shader("merge")
                            ...
                            .when(shadows_enabled, |f| {
                                f.input("shadow_map", "shadow", "depth")
                            })
                            .when(ssao_enabled, |f| {
                                f.input("g_ssao", "blur_ssao_vert", "blur_ssao")
                            })
                            .pre(move |ctx| {
                                let view_matrix_inv = ctx
                                    .var::<Matrix4<f32>>("view_matrix")
                                    .and_then(|v| v.invert())
                                    .expect("Missing view matrix");
                                let shadow_projection =
                                    ctx.var::<Matrix4<f32>>("shadow_projection").cloned();
                                let shadow_view_matrix =
                                    ctx.var::<Matrix4<f32>>("shadow_view_matrix").cloned();
                                let p = ctx.program("merge");
                                shadow_projection.map(|m| {
                                    p.uniform("shadow_projection").map(|v| v.set_matrix4(&m))
                                });
                                shadow_view_matrix
                                    .map(|m| p.uniform("shadow_matrix").map(|v| v.set_matrix4(&m)));
                                p.uniform("view_matrix_inv")
                                    .map(|v| v.set_matrix4(&view_matrix_inv));
                            })
                    })
                    .clear_flags(gl::BufferBit::COLOR)
                    .flag(PassFlag::empty())
            })
            .pass("focused_effect", |_, p| {
                p.final_color()
                    .runtime_enable("focused")
                    .fullscreen(|f| {
                        f.shader("focused_effect")
                            .input_final("g_color")
                            .input("g_position", "base", "position")
                            .pre(move |ctx| {
                                let time = *ctx.var::<f64>("time").expect("Missing timer");
                                let view_matrix_inv = ctx
                                    .var::<cgmath::Matrix4<f32>>("view_matrix")
                                    .and_then(|v| v.invert())
                                    .expect("Missing view matrix");
                                let area = *ctx
                                    .var::<Option<Bound>>("focused_area")
                                    .and_then(|v| v.as_ref())
                                    .expect("Missing focused_area");
                                let p = ctx.program("focused_effect");
                                p.uniform("time").map(|v| v.set_float(time as f32));
                                p.uniform("focused_area").map(|v| {
                                    v.set_int4(area.min.x, area.min.y, area.max.x, area.max.y)
                                });
                                p.uniform("view_matrix_inv")
                                    .map(|v| v.set_matrix4(&view_matrix_inv));
                            })
            ...
            .pass("fxaa", |_, p| {
                p.enabled(fxaa_enabled)
                    .final_color()
                    .fullscreen(|f| f.shader("fxaa").input_final("g_color"))
                    .clear_flags(gl::BufferBit::COLOR)
                    .flag(PassFlag::empty())
            })
            .pass("scale", |_, p| {
                p.enabled(render_scaling)
                    .final_color()
                    .fullscreen(|f| f.shader("scale").input_final("g_color"))
                    .clear_flags(gl::BufferBit::COLOR)
                    .flag(PassFlag::empty())
            })
            .pass("pause_effect", |_, p| {
                p.final_color()
                    .runtime_enable("paused")
                    .fullscreen(|f| {
                        f.shader("pause_effect")
                            .input_final("g_color")
                            .pre(move |ctx| {
                                let time = *ctx.var::<f64>("time").expect("Missing timer");
    ...
    pub fn update_image(&mut self, name: ResourceKey<'_>, width: u32, height: u32, data: Vec<u8>) {
        self.ui_renderer
            .update_image(&mut self.state.global_atlas, name, width, height, data)
    }
    ...
    pub fn tick(
        ...
        let projection = RenderState::get_projection_matrix(width, height, self.camera.zoom);
        let frustum = Frustum::from_matrix(projection * view_matrix);
        let (tx, ty, ts) = self
            .terrain
            .as_ref()
            .map_or(Default::default(), |v| v.get_render_bounds(&frustum));
        let shadow_view_matrix = RenderState::get_view_matrix(tx, ty, 1.0, self.shadow_rotation);
        ...
        self.global_atlas
            .texture
            .bind(gl::TextureTarget::Texture2DArray);
        ...
        }
        {
            ...
            self.pipeline
                ...
                .draw(width, height, |ctx, flag| {
                    state.draw(ctx, *flag, &mut entities, delta)
                });
    ...
    pub fn set_level(&mut self, level: &level::Level) {
        self.terrain = None;
        self.terrain = Some(terrain::Terrain::new(
            &self.state.log,
    ...
    pub fn start_selection<'a>(
        &mut self,
        owner_id: player::Id,
        level: &mut level::Level,
        room: assets::ResourceKey<'a>,
        x: i32,
        y: i32,
    ) {
        let bound = Bound::new(Location::new(x, y), Location::new(x, y));
        let verts = RenderState::gen_selection_verts(
            &mut self.state.global_atlas,
            assume!(self.state.log, self.state.terrain.as_mut()),
            level,
            room,
            bound,
            owner_id,
        ...
        let mdl = self
            .selection
            .as_mut()
            .expect("Missing selection after setting");
        mdl.selection_model
            .uniforms
            .insert("u_cycle", model::UniformValue::Float(0.0));
        mdl.selection_model.uniforms.insert(
            "u_textures",
            model::UniformValue::Int(GLOBAL_TEXTURE_LOCATION as i32),
        );
        mdl.selection_model.uniforms.insert(
            "shadow_map",
            model::UniformValue::Int(SHADOW_MAP_LOCATION as i32),
        );
        mdl.selection_model.uniforms.insert(
            "shadow_matrix",
            model::UniformValue::Matrix4(cgmath::SquareMatrix::identity()),
        );
        mdl.selection_model.uniforms.insert(
            "shadow_projection",
            model::UniformValue::Matrix4(cgmath::SquareMatrix::identity()),
        ...
        if let Some(selection) = self.state.selection.as_mut() {
            ...
            if selection.current != pos {
                ...
                selection
                    ...
                    .uniforms
                    .insert("u_cycle", model::UniformValue::Float(0.0));
                selection.selection_model.uniforms.insert(
                    "u_textures",
                    model::UniformValue::Int(GLOBAL_TEXTURE_LOCATION as i32),
                );
                selection.selection_model.uniforms.insert(
...
impl RenderState {
    fn draw(
        ...
        flag: PassFlag,
        entities: &mut Option<&mut ecs::Container>,
        delta: f64,
    ) {
        ...
        if !flag.contains(PassFlag::NO_CURSOR) {
            if let (Some(shadow_view_matrix), Some(shadow_projection)) =
                (shadow_view_matrix, shadow_projection)
            {
                ...
                if let Some(sel) = self.selection.as_mut() {
                    ...
                    *assume!(
                        self.log,
                        sel.selection_model.uniforms.get_mut("shadow_matrix")
                    ) = model::UniformValue::Matrix4(shadow_view_matrix);
                    *assume!(
                        self.log,
                        sel.selection_model.uniforms.get_mut("shadow_projection")
                    ) = model::UniformValue::Matrix4(shadow_projection);
                    *assume!(self.log, sel.selection_model.uniforms.get_mut("u_cycle")) =
                        model::UniformValue::Float(sel.cycle as f32);
    ...
    pub fn handle_key_action(&mut self, action: keybinds::KeyAction) {
        ...
        match action {
            ...
            RenderCameraDown | RenderCameraUp | RenderCameraLeft | RenderCameraRight => {
    ...
    pub fn preload_texture(&mut self, tex: assets::ResourceKey<'_>) {
        Self::texture_info_for(&self.log, &self.asset_manager, &mut self.global_atlas, tex);
    }

    fn texture_info_for(
        log: &Logger,
        asset_manager: &assets::AssetManager,
        atlas: &mut GlobalAtlas,
        tex: assets::ResourceKey<'_>,
    ) -> (i32, atlas::Rect) {
    ...
    fn place_texture_atlas(
        target_atlas: &mut GlobalAtlas,
        img: image::ImageFuture,
    ) -> (i32, atlas::Rect) {
    ...
    fn upload_texture(
        texture: &gl::Texture,
        tex: Option<image::Image>,
        atlas: i32,
        rect: atlas::Rect,
    ) {
        let data = tex.map_or_else(
            || vec![255; (rect.width * rect.height * 4) as usize],
            |v| v.data,
        );
        texture.bind(gl::TextureTarget::Texture2DArray);
...
impl RenderState {
    ...
    fn compute_entities(&mut self, entities: &mut ecs::Container, frustum: &Frustum, delta: f64) {
        // Static models
             ...
             s_model: ecs::Read<entity::StaticModel>| {
                for (e, attachment) in em.group_mask(&attachment, |m| m.and(&e_model)) {
                    ...
                    let (base, bone) = {
                        ...
                        if let Some(dyn_offset) =
                            other_model.entity_map.get(&attachment.target).cloned()
                        {
                            ...
                            (
                                r#dyn.matrix,
                                other_model.bone_node_info
                                    [dyn_offset * model.bones.len() + bone_id],
                            )
                        } else {
                    ...
                    if s_model.get_component(e).is_some() {
                        let model = if let Some(m) = self
                            .static_info
                            .models
                            .get_mut(&ModelKeyBorrow(model.name.borrow(), tex))
                        {
                            m
                        } else {
                            continue;
                        ...
                        if let Some(r#dyn) = model.entity_map.get(&e).cloned() {
                            {
                                ...
                                r#dyn.matrix =
                                    base * bone * attachment.offset * model.info.transform;
                            }
                            model.matrix_buffer.bind(gl::BufferTarget::Array);
                            model.matrix_buffer.set_data_range(
                                gl::BufferTarget::Array,
                                &model.dyn_info[r#dyn..=r#dyn],
                                r#dyn as i32,
                            );
                        }
                    } else {
        ...
        ER: for<'a> EntityRender<'a>,
    {
        ...
        entities.with(
            |em: EntityManager<'_>,
             ...
             params: ER::Params| {
                let mask = component.mask().and(&model).and(&pos);

                let mut ents = em.iter_mask(&mask).collect::<SmallVec<[_; 32]>>();

                ents.retain(|v| {
                    let pos = assume!(log, pos.get_component(*v));
                    let pos = cgmath::Vector3::new(pos.x, pos.y, pos.z);
                    if let Some(size) = size.get_component(*v) {
                        ...
                        frustum.contains_aabb(pos - dims, pos + dims)
                    } else {
...
impl RenderState {
    ...
    fn get_view_matrix(x: f32, y: f32, zoom: f32, rotation: cgmath::Deg<f32>) -> Matrix4<f32> {
        use cgmath::{Basis3, Deg, Matrix3, Vector3};
        Matrix4::from_scale(75.0 * zoom)
            * Matrix4::from(Matrix3::from(Basis3::from_angle_x(Deg(-35.264f32))))
            * Matrix4::from(Matrix3::from(Basis3::from_angle_y(
                Deg(-45.0f32) + rotation,
            )))
            * Matrix4::from_translation(Vector3::new(-x, 0.0, -y))
    }
        ...
        if self.mouse_sprite != sprite {
            let mut img = assume!(
                ...
                assume!(
                    self.log,
                    self.asset_manager
                        .loader_open::<image::Loader>(sprite.clone())
                )
                .wait_take_image()
            );
    ...
    fn gen_selection_verts<'a>(
        ...
        for loc in bound {
            ...
            for vert in &mut tverts {
                vert.brightness = if tile_valid { 1.0 } else { 0.0 };
            }
...
#[derive(Debug, PartialEq, Eq, PartialOrd, Hash, Clone)]
pub(crate) struct ModelKeyBorrow<'a>(
    pub assets::ResourceKey<'a>,
    pub Option<assets::ResourceKey<'a>>,
);
