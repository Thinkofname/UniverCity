...
impl Icons {
    pub fn new(log: &Logger, ctx: &mut pipeline::Context<'_>) -> Icons {
        ...
        attrib_position.vertex_pointer(
            ...
            gl::Type::Float,
            false,
            mem::size_of::<UniqueData>() as i32,
            0,
        );
        attrib_position.divisor(1);
        attrib_size.enable();
        attrib_size.vertex_pointer(
            2,
             ...
             color: ecs::Read<Color>| {
                ...
                if !data.is_empty() {
                    ...
                    prog.uniform("u_view_matrix")
                        .map(|v| v.set_matrix4(&matrix));
                    prog.uniform("u_view_up")
                        .map(|v| v.set_float3(up.x, up.y, up.z));
                    prog.uniform("u_view_right")
                        .map(|v| v.set_float3(right.x, right.y, right.z));
                    ...
                    self.icons.dynamic_buffer.set_data(
