use crate::util::{FNVMap, Location};
use std::mem;
...
use crate::server::level::{self, room, SECTION_SIZE};
...
impl Terrain {
    ...
    pub(super) fn update(
        ...
        for room_id in level.room_ids() {
            ...
            if let Some(virt) = room.building_level.as_ref() {
                if dirty {
                    ...
                    let wall_height = if lower_walls { 4.0 / 16.0 } else { 1.0 };
                    ...
                    if new {
                        edit.model.uniforms.insert(
                            "u_textures",
                            model::UniformValue::Int(super::GLOBAL_TEXTURE_LOCATION as i32),
                        );
                        edit.model.uniforms.insert(
                            "shadow_map",
                            model::UniformValue::Int(super::SHADOW_MAP_LOCATION as i32),
                        );
                        edit.model.uniforms.insert(
                            "shadow_matrix",
                    ...
                    edit.model.uniforms.insert(
                        "u_cycle",
                        model::UniformValue::Float(match room.state {
        ...
        for section in &mut self.sections {
            ...
            let min_x = if section.x == 0 {
                -1
            } else {
                (section.x * SECTION_SIZE) as i32
            };
            let min_y = if section.y == 0 {
                -1
            } else {
                (section.y * SECTION_SIZE) as i32
            };
            ...
            for x in min_x..max_x {
                ...
                for y in min_y..max_y {
                    ...
                    if level
                        .get_tile_flags(loc)
                        .contains(level::TileFlag::BUILDING)
                    {
                        continue;
                    }
                    if x >= 0 && y >= 0 {
            ...
            if section.max_count < count {
                section
                    .buffer
                    .set_data(gl::BufferTarget::Array, &data, gl::BufferUsage::Dynamic);
                section.max_count = count;
            } else {
                section
                    .buffer
                    .set_data_range(gl::BufferTarget::Array, &data, 0);
            }
            section.count = count;
        }
        ...
        for id in level.room_ids() {
            ...
            if let Some(bounds) = room.get_placement_bounds() {
                ...
                let guide = self
                    .placement_guides
                    .entry(id)
                    .or_insert_with(|| PlacementGuide {
                        model: model::Model::new(ctx, "placement", vec![], vec![]),
                ...
                guide.bounds = bounds;
                {
                    ...
                    guide.texture.set_parameter::<gl::TextureMinFilter>(
                        gl::TextureTarget::Texture2D,
                        gl::TextureFilter::Nearest,
                    );
                    guide.texture.set_parameter::<gl::TextureMagFilter>(
                        gl::TextureTarget::Texture2D,
                        gl::TextureFilter::Nearest,
                    );
                    guide.texture.set_parameter::<gl::TextureWrapS>(
                        gl::TextureTarget::Texture2D,
                        gl::TextureWrap::ClampToEdge,
                    );
                    guide.texture.set_parameter::<gl::TextureWrapT>(
                        gl::TextureTarget::Texture2D,
                        gl::TextureWrap::ClampToEdge,
                    );
                    guide
                        .texture
                        .set_parameter::<gl::TextureBaseLevel>(gl::TextureTarget::Texture2D, 0);
                    guide
                        .texture
                        .set_parameter::<gl::TextureMaxLevel>(gl::TextureTarget::Texture2D, 0);
        ...
        L: level::LevelView,
    {
        ...
        let t_room = t_room_i.map(|v| {
            assume!(
                log,
                asset_manager.loader_open::<room::Loader>(v.key.borrow())
            )
        });
        ...
        for &(direction, face_a, face_b, tex_func, tex_func2, nor_a, nor_b) in &walls {
            ...
            if level
                .get_tile_flags(loc.shift(direction))
                .contains(level::TileFlag::BUILDING)
            {
                continue;
            }
            ...
            if let Some(wall_info) = wall_info {
                ...
                let t_shifted_room = t_shifted_room_i.map(|v| {
                    assume!(
                        log,
                        asset_manager.loader_open::<room::Loader>(v.key.borrow())
                    )
                });
                let b_wall = if let Some(wall) =
                    t_shifted_room.as_ref().and_then(|v| v.wall.as_ref())
                {
                ...
                let (tex_f, tex_b, tex_s, tex_top) = match (&a_wall, &b_wall) {
                    (&Some(ref a_wall), &None) => (
                        a_wall.1.borrow(),
                        a_wall.1.borrow(),
                        a_wall.1.borrow(),
                        a_wall.2.borrow(),
                    ),
                    (&None, &Some(ref b_wall)) => (
                        b_wall.1.borrow(),
                        b_wall.1.borrow(),
                        b_wall.1.borrow(),
                        b_wall.2.borrow(),
                    ),
                    (&Some(ref a_wall), &Some(ref b_wall)) => {
                        if b_wall.0 {
                            (
                                a_wall.1.borrow(),
                                b_wall.1.borrow(),
                ...
                let texture_top_id =
                    Self::get_texture_id(log, asset_manager, target_atlas, tex_top);
                    ...
                    if let level::TileWallFlag::Window(id) = wall_info.flag {
                        if id as usize <= windows.len() {
                            for i in windows.len() as u8..=id {
                                ...
                                windows.push(window::Model::load(
                                    log,
                                    asset_manager,
                                    target_atlas,
                                    key,
                ...
                if direction == Direction::West {
                    match (
                        ...
                        // ones aren't
                        (Some(_), None, Some(_)) | (Some(_), Some(_), None) => max = 1.0,
                        _ => {}
                    }
                    match (
                        ...
                        // Override if we are part of a wall and the south
                        // ones aren't
                        (Some(_), None, Some(_)) | (Some(_), Some(_), None) => min = 0.0,
                        _ => {}
                    }
                } else {
                    match (
                        ...
                        (None, None, Some(_)) | (None, Some(_), None) => min = -1.0 / 16.0,
                    ...
                    match (
                        ...
                        (None, None, Some(_)) | (None, Some(_), None) => max = 17.0 / 16.0,
                ...
                let (shift, size, size_full) = if mul_y == 1 {
                    (
                        (min, 0.0, 0.0),
                        (max - min, wall_height, 2.0 / 16.0),
                        (max - min, wall_height, 1.0),
                    )
                } else {
                    (
                        (0.0, 0.0, min),
                        (2.0 / 16.0, wall_height, max - min),
                        (1.0, wall_height, max - min),
                    )
                };
                ...
                Self::make_wall(
                    ...
                    texture_f_id,
                    (cx, 0.0, cy),
                    (shift_x + min * mulf_y, 0.0, shift_y + min * mulf_x),
                    size_full,
                    nor_a_r,
                    tex_func,
                );
                Self::make_wall(
                    ...
                    (cx, 0.0, cy),
                    (
                        shift_x + (2.0 / 16.0) * mulf_x + min * mulf_y,
                        0.0,
                        shift_y + (2.0 / 16.0) * mulf_y + min * mulf_x,
                    ),
                    size_full,
                    nor_a,
                    tex_func,
                ...
                if cull_check(loc, Direction::from_offset(-mul_y, -mul_x), direction) {
                    Self::make_wall(
                        ...
                        texture_s_id,
                        (cx, 0.0, cy),
                        (shift_x + shift.0, 0.0, shift_y + shift.2),
                        size,
                        nor_b_r,
                        tex_func2,
                    );
                    ...
                    if wall_height >= 1.0
                        && !bounds.in_bounds(loc.shift(Direction::from_offset(-mul_y, -mul_x)))
                    {
                        let mut size = size;
                        ...
                        Self::make_wall(
                            ...
                            texture_s_id,
                            (cx, 0.0, cy),
                            (shift_x + shift.0, wall_height, shift_y + shift.2),
                            size,
                            nor_b,
                            tex_func2,
                        );
                ...
                if cull_check(loc, Direction::from_offset(mul_y, mul_x), direction) {
                    Self::make_wall(
                        ...
                        (cx, 0.0, cy),
                        (
                            size_full.0 * mulf_y + shift_x + shift.0,
                            0.0,
                            size_full.2 * mulf_x + shift_y + shift.2,
                        ),
                        size,
                        nor_b,
                        tex_func2,
        ...
        V: Iterator<Item = &'a Vertex>,
    {
        for vert in verts {
            ...
            data.push(GLVertex {
                ...
                texture_offset_x: texture.1.x as f32
                    + tx.max(mw).min(1.0 - mw) * texture.1.width as f32,
                texture_offset_y: texture.1.y as f32
                    + ty.max(mh).min(1.0 - mh) * texture.1.height as f32,
                brightness: 1.0,
            });
        }
    }
    ...
    pub fn get_render_bounds(&self, frustum: &Frustum) -> (f32, f32, f32) {
        ...
        for sec_x in 0..((self.width + 3) / 4) {
            for sec_y in 0..((self.height + 3) / 4) {
                ...
                let min = cgmath::Vector3::new(sx, 0.0, sy);
                let max = cgmath::Vector3::new(sx + 4.0, 2.5, sy + 4.0);
        ...
        (total_x / render_count, total_y / render_count, size)
    }
    ...
    pub fn draw(
    ...
    ) {
        {
            let prog = ctx.program("terrain");
            prog.use_program();
            prog.uniform("view_matrix")
                .map(|v| v.set_matrix4(view_matrix));
            prog.uniform("projection_matrix")
                .map(|v| v.set_matrix4(projection));
            prog.uniform("u_textures")
                .map(|v| v.set_int(super::GLOBAL_TEXTURE_LOCATION as _));
            prog.uniform("shadow_map")
                .map(|v| v.set_int(super::SHADOW_MAP_LOCATION as _));
        ...
        if shadow_view_matrix.is_some() {
            for placement in self.placement_guides.values_mut() {
                ...
                placement
                    .model
                    .uniforms
                    .entry("u_placement_map")
                    .or_insert(model::UniformValue::Int(0));
        ...
        for edit in self.edit_sections.values_mut() {
            ...
            if let (Some(sv), Some(sp)) = (shadow_view_matrix, shadow_projection) {
                *assume!(self.log, edit.model.uniforms.get_mut("shadow_matrix")) =
                    model::UniformValue::Matrix4(*sv);
                *assume!(self.log, edit.model.uniforms.get_mut("shadow_projection")) =
                    model::UniformValue::Matrix4(*sp);
            }
    ...
    fn get_texture_id(
        ...
        super::RenderState::texture_info_for(log, asset_manager, target_atlas, tex)
    }
}
