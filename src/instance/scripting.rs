...
use crate::server::lua::{self, Lua, Ref, Table};
...
impl lua::LuaUsable for LuaRoom {
    ...
    fn fields(t: &lua::TypeBuilder) {
        t.field(
            "get_size",
            lua::closure1(|_lua, this: Ref<LuaRoom>| (this.area.width(), this.area.height())),
        );
        t.field(
            "get_properties",
        ...
        t.field(
            "get_bounds",
            lua::closure1(|_lua, this: Ref<LuaRoom>| -> UResult<_> {
                Ok((
                    this.area.min.x,
                    this.area.min.y,
                    this.area.width(),
                ...
                Ok(room
                    ...
                    .and_then(|v| {
                        ...
                        lua::with_table_serializer(lua, |se| {
                            server::serde_transcode::transcode(&mut de, se)
                        })
                        .ok()
                    })
        ...
        t.field(
            "get_position",
            lua::closure1(|_lua, this: Ref<LuaRoom>| -> (i32, i32) {
...
impl RunningChoices {
    pub fn new(log: &Logger, assets: &AssetManager) -> RunningChoices {
        ...
        let student_idle_scripts = choices.student_idle.iter().map(|v| v.1.clone()).collect();
        RunningChoices {
...
pub(super) fn load_choice_state(
    ...
    let state = assume!(
        log,
        with_table_serializer(scripting, |se| serde_transcode::transcode(&mut de, se))
    );
            ...
            let (lua_room, room_entities) = entities.with(
                |_em: EntityManager<'_>,
                 ...
                 rc: ecs::Read<RoomController>| {
                    ...
                    let room_entities = rc
                        .get_component(room.controller)
                        .into_iter()
...
pub fn client_tick(log: &Logger, entities: &mut Container, scripting: &script::Engine) {
    free_roam::tick::<Types, _>(log, entities, scripting, &mut (), "client_handler");
}
    ...
    fn fields(t: &lua::TypeBuilder) {
        ...
        t.field(
            ...
            lua::closure1(|lua, this: Ref<LuaEntity>| {
                ...
                entities
                    .get_component::<server::entity::pathfind::PathInfo>(this.entity)
                    .is_some()
            }),
        );
        t.field(
        ...
        t.field(
            "get_position",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> errors::Result<(f64, f64)> {
                let entities = lua.read_borrow::<ecs::Container>();
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
        ...
        t.field(
            ...
            lua::closure1(|lua, this: Ref<LuaEntity>| {
                let entities = lua.read_borrow::<ecs::Container>();
                if let Some(state) =
                    entities.get_component::<server::entity::StateData>(this.entity)
                {
        ...
        t.field(
            "get_model",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> errors::Result<Ref<String>> {
        ...
        t.field(
            "set_model",
            lua::closure2(
                |lua, this: Ref<LuaEntity>, model: Ref<String>| -> errors::Result<()> {
                    let mut entities = lua.write_borrow::<ecs::Container>();
                    let key = match ResourceKey::parse(&model) {
                        Some(val) => val,
                        None => bail!("Invalid resource key"),
        ...
        t.field(
            "set_model_texture",
            lua::closure2(
                |lua, this: Ref<LuaEntity>, tex: Ref<String>| -> errors::Result<()> {
                    let mut entities = lua.write_borrow::<ecs::Container>();
                    ...
                    if let Some(mdl) =
                        entities.get_component_mut::<entity::ModelTexture>(this.entity)
                    {
                        if mdl.name != key {
                            mdl.name = key.into_owned();
                        }
        ...
        t.field(
            "get_animation",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> errors::Result<_> {
                let mut entities = lua.write_borrow::<ecs::Container>();
                if let Some(ani) = entities.get_component_mut::<entity::AnimatedModel>(this.entity)
                {
                    Ok(ani.current_animation().map(|v| Ref::new_string(lua, v)))
                } else {
                    bail!("Invalid entity")
                }
            }),
        ...
        t.field(
            "play_animation",
            lua::closure2(
                |lua, this: Ref<LuaEntity>, name: Ref<String>| -> errors::Result<()> {
                    let mut entities = lua.write_borrow::<ecs::Container>();
                    if let Some(ani) =
                        entities.get_component_mut::<entity::AnimatedModel>(this.entity)
                    {
            ...
            lua::closure4(
                |lua,
                 ...
                 -> UResult<_> {
                    ...
                    Ok(Ref::new(lua, LuaAttachment { attachment }))
                },
            ),
        );
            ...
            lua::closure5(
                |lua,
                 ...
                 -> errors::Result<_> {
                    ...
                    entities.add_component::<server::entity::TargetPosition>(
                        this.entity,
                        server::entity::TargetPosition {
                            x: x as f32,
                            y: y as f32,
                            z: z as f32,
                            ticks,
        ...
        t.field(
            "rotate",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, target: f64, ticks: f64| -> errors::Result<_> {
                    if !this.client_entity {
                        bail!("Can't rotate non-client entities")
                    }
                    let mut entities = lua.write_borrow::<ecs::Container>();
                    entities.add_component::<server::entity::TargetRotation>(
                        this.entity,
                        server::entity::TargetRotation {
...
impl lua::LuaUsable for LuaAttachment {
    fn fields(t: &lua::TypeBuilder) {
        t.field(
            "remove",
            lua::closure1(|lua, this: Ref<LuaAttachment>| {
                let mut entities = lua.write_borrow::<ecs::Container>();
