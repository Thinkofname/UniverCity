...
impl state::State for BaseState {
    ...
    fn added(
        ...
        *instance = self.instance.take();
        ...
        state.renderer.set_camera(
            instance.level.width as f32 / 2.0,
            instance.level.height as f32 / 2.0,
        );
        state
            .ui_manager
            .set_script_engine(&state.audio, &instance.scripting);
    ...
    fn active(
        ...
        self.hud = Some(
            state
                .ui_manager
                .create_node(ResourceKey::new("base", "hud")),
        );
    ...
    fn inactive(&mut self, instance: &mut Option<GameInstance>, state: &mut crate::GameState) {
        ...
        for not in &mut instance.notifications {
            not.description
                .parent()
                .map(|v| v.remove_child(not.description.clone()));
            if not.ui.is_some() {
                not.ui = None;
            }
        }
    ...
    fn removed(&mut self, instance: &mut Option<GameInstance>, state: &mut crate::GameState) {
        ...
        if let Some(scr) = screenshot.as_ref() {
            ...
            state
                .renderer
                .tick(Some(&mut entities), None, state.delta, width, height);
        ...
        if let Some(waiter) = shutdown_waiter {
            if let Err(err) = waiter.recv_timeout(time::Duration::from_secs(60)) {
                error!(
                    state.global_logger,
                    "Failed to wait for server shutdown: {:?}", err
                );
            }
        }
        state
            .keybinds
            .remove_collection(keybinds::KeyCollection::Game);
    }
    ...
    fn tick(
        ...
        if instance.player.money != self.current_money || self.first_frame {
            ...
            if let Some(text) = query!(hud, stats > money > @text).next() {
        ...
        if instance.player.rating != self.current_rating as i16 || self.first_frame {
            if self.first_frame {
                ...
                self.current_rating += (f64::from(instance.player.rating) - self.current_rating)
                    .signum()
                    * state.delta;
            }
            if let Some(rating_bar) = query!(hud, stats > rating > rating_clip).next() {
        ...
        for (i, not) in instance.notifications.iter_mut().enumerate() {
            if not.ui.is_none() {
                not.index = i as f64;
                if let Some(target) = query!(hud, notifications).next() {
                ...
                KeepReason::EntityOwned(e) => {
                    ...
                    if !instance.entities.is_valid(e)
                        || !instance
                            ...
                            .map_or(false, |v| v.player_id == player_id)
                    {
                        state
                            .ui_manager
                            .events
                            .borrow_mut()
                            .emit(CloseNotification(not.id));
                    }
                }
            }
        }
        ...
        if let Some(fly) = self.current_fly.take() {
            fly.0.set_property(
                "target_index",
                fly.1.get_property::<i32>("index").unwrap_or(0),
            );
        ...
        if let State::EditRoom { .. } | State::EditEntity { .. } = instance.player.state {
            ...
            if let Some(hroom) = self.highlighted_room.as_mut() {
                ...
                if hroom.time > 60.0 * 0.35 && !hroom.showed && hroom.requested.is_none() {
                    ...
                    hroom.requested = Some(
                        instance
                            .request_manager
                            .request(player::RoomDetails { room_id: hroom.id }),
                    );
                    ...
                    if cur.map_or(true, |v| v.starts_with("room_")) {
                        if let Some(room_info) = instance.level.try_room_info(hroom.id) {
                            let ty = assume!(
                                ...
                                instance
                                    .asset_manager
                                    .loader_open::<crate::server::level::room::Loader>(
                                        room_info.key.borrow()
                                    )
                            );
                            if ty.allow_edit && room_info.owner == instance.player.id {
                                let content = hroom.content.take().unwrap_or_else(|| {
                                    node! {
                                        content {
                                            @text(ty.name.clone())
                                        }
        ...
        if let Some(day) = query!(hud, day > @text).next() {
            let sim_day = NaiveDate::from_ymd(2018, 1, 1)
                + chrono::Duration::days(i64::from(instance.day_tick.day));
            day.set_text(format!(
                "{}{} {}",
                sim_day.format("%B %e"),
                match sim_day.day() {
        ...
        if let Some(period) = query!(hud, period > @text).next() {
            ...
            period.set_text(format!(
                "{}{} Period",
                activity_slot + 1,
                match activity_slot + 1 {
                    1 => "st",
                    2 => "nd",
                    3 => "rd",
                    _ => "th",
        ...
        if let Some(time) = query!(hud, time > @text).next() {
            ...
            let minutes = (tick * 2 * 60 * NUM_TIMETABLE_SLOTS as i32)
                / (LESSON_LENGTH * NUM_TIMETABLE_SLOTS as i32);
    ...
    fn ui_event(
                    ...
                    if hroom.id == pck.room_id {
                        ...
                        if let Some(script) = ty.controller.as_ref() {
                            ...
                            match instance
                                ...
                                .invoke_function::<_, Ref<ui::NodeRef>>(
        ...
        evt.handle_event::<ClickNotification, _>(|c| {
            ...
            if let Some(not) = instance.notifications.iter_mut().find(|v| v.id == c.0) {
        ...
        evt.handle_event::<CloseNotification, _>(|c| {
            instance.notifications.retain(|v| {
                if v.id == c.0 {
                    if let Some(ui) = v.ui.clone() {
                        if let Some(target) = query!(
                            assume!(state.global_logger, self.hud.as_ref()),
                            notifications
                        )
                        .next()
                        {
        ...
        evt.handle_event::<ChatMessage, _>(|ChatMessage(msg)| {
            match msg.as_str() {
                ...
                _ => {
                    let _ = instance.ensure_send(packet::ChatMessage { message: msg });
                }
            };
        });
        action
        ...
        match action {
            ...
            BeginChat => {
                if query!(hud, textbox(id = "chat_sendbox")).next().is_none() {
                    ...
                    txt.set_property(
                        "on_key",
                        ui::MethodDesc::<ui::KeyUpEvent>::native(move |evts, node, param| {
            ...
            InspectMember => {
                if let Some(entity) =
                    find_entity_at(&state.renderer, &mut instance.entities, mouse_pos)
                {
                    if !instance
                        .entities
                        .get_component::<Owned>(entity)
                        .map_or(false, |v| v.player_id == instance.player.id)
                    {
                    ...
                    if instance.entities.get_component::<Paid>(entity).is_some()
                        || instance
                            .entities
                            .get_component::<StudentController>(entity)
                            .is_some()
                    {
                        // Student
    ...
    fn mouse_move_ui(
        ...
        if let Some(room) = self.highlighted_room.take() {
            state
                .ui_manager
                .hide_tooltip(&format!("room_{:?}", room.id));
        }
        state::Action::Nothing
    }
    ...
    fn mouse_move(
        ...
        if let Some(entity) = find_entity_at(&state.renderer, &mut instance.entities, mouse_pos) {
            let ty = {
                instance
                    ...
                    .and_then(|v| {
                        state
                            .asset_manager
                            .loader_open::<Loader<entity::ClientComponent>>(v.key.borrow())
                            .ok()
                    })
            };
            if let Some(highlight) = ty.as_ref().and_then(|v| v.highlight.as_ref()) {
                ...
                if let Some(HighlightedRoom {
                    id: room,
                    showed: true,
                    ..
                }) = self.highlighted_room.take()
                {
                ...
                if let Some(label) = highlight.label.as_ref() {
                    ...
                    if can_interact {
                        ...
                        let buttons = state
                            .keybinds
                            .keys_for_action(keybinds::KeyAction::InspectMember)
            ...
            if let Some(entity) = self.highlighted_entity.take() {
                ...
                state
                    .ui_manager
                    .hide_tooltip(&format!("entity_{:?}", entity));
            }

            let (lx, ly) = state.renderer.mouse_to_level(mouse_pos.0, mouse_pos.1);
            if let Some(room) = instance
                .level
                .get_room_owner(Location::new(lx as i32, ly as i32))
            {
                if let Some(hroom) = self.highlighted_room.as_mut() {
                    if hroom.id == room {
                        ...
                        if dx * dx + dy * dy > 10 * 10 {
                            ...
                            if hroom.showed {
                                state
                                    .ui_manager
                                    .hide_tooltip(&format!("room_{:?}", hroom.id));
                            }
                            hroom.showed = false;
                            hroom.requested = None;
                            hroom.loc = mouse_pos;
                        }
                        state.ui_manager.move_tooltip(
                            &format!("room_{:?}", hroom.id),
                            mouse_pos.0,
                            mouse_pos.1,
                        );
            ...
            em.group_mask((&position, &size), |m| m.and(&living))
                .find(|&(_e, (pos, size))| {
                    ...
                    let bound = AABB {
                        ...
                        max: vp
                            + cgmath::Vector3::new(size.width / 2.0, size.height, size.depth / 2.0),
                    };
