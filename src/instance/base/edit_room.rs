...
fn update_room_ui(
    ...
    if let (Some(name), Some(limited)) = (
        query!(ui, room_name > @text).next(),
        query!(ui, limited > @text).next(),
    ) {
        if let Some(room_id) = room_id {
            ...
            if !room.state.is_done() || room.owner != instance.player.get_uid() || !rinfo.allow_edit
            {
                ...
                } else if !room.controller.is_invalid()
                    && instance
                        .entities
...
impl state::State for EditRoomState {
    ...
    fn active(
        ...
        let ui = state
            .ui_manager
            .create_node(ResourceKey::new("base", "manage/edit_room"));
        self.ui = Some(ui.clone());
        state
            .keybinds
            .add_collection(keybinds::KeyCollection::EditRoom);
        state
            .renderer
    ...
    fn inactive(&mut self, _instance: &mut Option<GameInstance>, state: &mut crate::GameState) {
        ...
        state
            .keybinds
            .remove_collection(keybinds::KeyCollection::EditRoom);
        state
            .renderer
        ...
        if let Some((room_id, req)) = self.request {
            network::RequestManager::handle_reply(evt, req, |res| {
                if let Some(room) = instance
                    .level
                    .try_room_info(res.room_id)
                    .filter(|v| !v.controller.is_invalid())
                {
                    if res.booked {
                        instance
                            .entities
                            .add_component(room.controller, entity::ClientBooked);
                    } else {
                        instance
                            .entities
                            .remove_component::<entity::ClientBooked>(room.controller);
                    }
                }
                update_room_ui(
                    &state.global_logger,
            ...
            if let Some(room_id) = room_id {
                if self.request.as_ref().map_or(true, |v| v.0 != room_id) {
                    self.request = Some((
                        ...
                        instance
                            .request_manager
                            .request(player::RoomBooked { room_id: room_id }),
                    ));
                }
            }
            update_room_ui(
        ...
        match action {
            ...
            SelectEditRoom => {
                ...
                if let Some(room_id) = instance.level.get_room_owner(loc) {
                    let mut cmd: Command = crate::server::command::EditRoom::new(room_id).into();
                    ...
                    match cmd.execute(
                        ...
                        Ok(_) => {
                            ...
                            return state::Action::Switch(Box::new(super::build::BuildRoom::new(
                                false,
                            )));
                        }
                        Err(server::errors::Error(
                            server::errors::ErrorKind::RoomNoFullOwnership,
                            _,
                        )) => {
                            let mut cmd: Command = EditRoomLimited::new(room_id).into();
