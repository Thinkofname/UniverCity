...
impl state::State for BuyRoomState {
        ...
        for module in assets.get_packs() {
            ...
            let rooms_raw: Vec<RoomInfoJson> = match serde_json::from_reader(room_file) {
                ...
                Err(err) => {
                    error!(
                        instance.log,
                        "Failed to parse rooms.json for pack {:?}: {}", module, err
                    );
                    continue;
                }
            };
            self.room_list.extend(rooms_raw.into_iter().map(|v| {
                RoomInfo {
    ...
    fn active(
        ...
        let ui = state
            .ui_manager
            .create_node(assets::ResourceKey::new("base", "buy/rooms"));
            ...
            if let Some(content) = query!(scroll_panel, scroll_panel > content).next() {
                for (i, room) in self.room_list.iter().enumerate() {
                    let ty = assume!(
                        ...
                        state
                            .asset_manager
                            .loader_open::<room::Loader>(room.room.borrow())
                    );
                    ...
                    if can_build {
                        ...
                        for req in &ty.requirements {
                            ...
                            ret.insert(
                                requirement_met.clone(),
                                req.check_requirement(&instance.level, instance.player.id),
                            );
                            ret.insert(
                                requirement_info.clone(),
                                Ref::new_string(&instance.scripting, s),
                            );
                ...
                if !ty.required_entities.is_empty() {
                    ...
                    for (entity, count) in &ty.required_entities {
                        let ety = assume!(
                            ...
                            state
                                .asset_manager
                                .loader_open::<Loader<entity::ClientComponent>>(entity.borrow())
                        );
