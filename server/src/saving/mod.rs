...
/// cannot.
pub fn can_load<F: FileSystem>(fs: &F, name: &str, ty: SaveType) -> UResult<bool> {
    let path = format!("{}.usav", name);
    if !fs.exists(&path) {
...
pub fn get_icon<F: FileSystem>(fs: &F, name: &str) -> Option<Vec<u8>> {
    let path = format!("{}.usav", name);
    if !fs.exists(&path) {
        return None;
    }
...
pub(crate) fn save_game<F: FileSystem>(
    ...
    let save_icon = icon.and_then(|v| v.capture());
    {
        ...
        let players_sf = entities.with(|_em: EntityManager, network_id: Read<NetworkId>| {
            SaveData::Players(
                players
                    .iter()
                    .map(|(k, v)| {
                        (
                            *k,
                            PlayerInfo {
                                ...
                                key: match v.key {
                                    #[cfg(feature = "steam")]
                                    player::PlayerKey::Steam(steam_id) => {
                                        PlayerKey::Steam(steam_id.raw())
                                    }
                                    #[cfg(not(feature = "steam"))]
                                    player::PlayerKey::Username(ref name) => {
                                        PlayerKey::Username(name.clone())
                                    }
                                },
                                money: v.money,
                                rating: v.rating,
                                ...
                                state: match v.state {
                                    crate::player::State::BuildRoom { active_room } => {
                                        PlayerState::BuildRoom { active_room }
                                    }
                                    crate::player::State::EditRoom { active_room } => {
                                        PlayerState::EditRoom { active_room }
                                    }
                                    // Don't bother with this
                                    crate::player::State::None
                                    | crate::player::State::EditEntity { .. } => PlayerState::None,
                                },
                                        ...
                                        (
                                            *id,
                                            SavableCourse {
                                                ...
                                                group: v.group.clone(),
                                                cost: v.cost,
                                                timetable: SavableCourse::conv_timetable(
                                                    &network_id,
                                                    &v.timetable,
                                                ),
...

        {
            ...
            entities.with(
                |em: EntityManager<'_>,
                 ...
                 -> UResult<()> {
                    for (e, living) in
                        em.group_mask(&living, |m| m.and_not(&frozen).and_not(&quitting))
                    {
                        let info = EntityInfo {
                            key: living.key.clone(),
                            variant: living.variant,
                            name: ((*living.name.0).to_owned(), (*living.name.1).to_owned()),
                            ...
                            vars: if let Some(vars) = student_vars
                                .get_custom(e)
                                .map(|v| v.remove_type())
                                .or_else(|| professor_vars.get_custom(e).map(|v| v.remove_type()))
                                .or_else(|| {
                                    office_worker_vars.get_custom(e).map(|v| v.remove_type())
                                })
                                .or_else(|| janitor_vars.get_custom(e).map(|v| v.remove_type()))
                            {
                                Some(VarsInfo {
                                    vars: vars.iter().map(|(k, v)| (k.to_owned(), v)).collect(),
                                })
                            } else {
                                None
                            },
                            ...
                            idle: if let Some(idle) = idle.get_component(e) {
                                Some(IdleInfo {
                                    ...
                                    current_choice: idle
                                        .current_choice
                                        .and_then(|v| {
                                            choices.student_idle.get_choice_name_by_index(v)
                                        })
                                        .map(|v| v.into_owned()),
                                })
                            } else {
                            ...
                            goto_room: goto_room
                                .get_component(e)
                                .map(|v| GotoRoomInfo { room_id: v.room_id }),
                            money: money.get_component(e).map(|v| MoneyInfo { money: v.money }),
                        };
        ...
        for ((player, idx), rc) in &running_choices.choices {
            ...
            let mut se = serde_cbor::ser::Serializer::new(vec![]);
            lua::with_table_deserializer(&result, |de| serde_transcode::transcode(de, &mut se))?;
            let data = se.into_inner();

            let name = assume!(log, choices.student_idle.get_choice_name_by_index(*idx));
            serde_cbor::to_writer(
                &mut f,
                &SaveData::IdleScript(*player, name.into_owned(), data),
            )?;
        }
...
fn init_players(
    ...
    for (id, player) in sf_players {
        ...
        let info = players
            .entry(id)
            .or_insert_with(|| crate::player::PlayerInfo::new(key, name, id, &staff_list));
...
fn load_game_generic(
    ...
    for sd in sf {
        match sd? {
            SaveData::Room(room) => {
                ...
                let id = assume!(
                    log,
                    level.place_room_id::<ServerEntityCreator, _>(
                        engine,
                        entities,
                        room_id,
                        owner,
            ...
            SaveData::RoomEntityState(id, state) => {
                ...
                if let Some(rc) = entities.get_component_mut::<RoomController>(rm.controller) {
                    rc.active_staff = state
                        .active_staff
                        .and_then(|v| snapshots.get_entity_by_id(v));
                    for (d, day) in state
                        .timetabled_visitors
                        .into_iter()
                        .zip(&mut rc.timetabled_visitors)
                    {
                        for (p, period) in d.into_iter().zip(day) {
                            period.extend(
                                p.into_iter().filter_map(|v| snapshots.get_entity_by_id(v)),
                            );
                        }
                    }
                }
            ...
            SaveData::Object(room_id, object) => {
                level.begin_object_placement::<_, ServerEntityCreator>(
                    room_id,
                    engine,
                    entities,
                    object.key.borrow(),
                ...
                if let Err(err) = level.move_active_object::<_, ServerEntityCreator>(
                    ...
                    match err {
                        ...
                        err => Err(err).chain_err(|| {
                            ErrorKind::Msg(format!("Failed to move object: {:?}", object.key))
                        })?,
                    }
                }
                level.finalize_object_placement::<_, ServerEntityCreator>(
                    room_id,
                    engine,
                    entities,
                    Some(object.version),
            ...
            SaveData::Entity(entity) => {
                let ty =
                    asset_manager.loader_open::<Loader<ServerComponent>>(entity.key.borrow())?;
                let variant = &ty.variants[entity.variant];
                let (fna, sna) = entity.name;
                let first_name = variant
                    .name_list
                    .first
                    .iter()
                    .find(|v| fna == ***v)
                ...
                let e = ty.create_entity(entities, entity.variant, Some((first_name, second_name)));
                entity
                    .network_id
                    .map(|v| snapshots.assign_network_id(entities, e, v));
                if let (Some(pos), Some(epos)) =
                    (entities.get_component_mut::<Position>(e), entity.position)
                {
                    pos.x = epos.x;
                    pos.y = epos.y;
                    pos.z = epos.z;
                }
                if let (Some(rot), Some(erot)) =
                    (entities.get_component_mut::<Rotation>(e), entity.rotation)
                {
                    rot.rotation = erot;
                }
                if let (Some(speed), Some(espeed)) =
                    (entities.get_component_mut::<MovementSpeed>(e), entity.speed)
                {
                ...
                if let Some(ro) = entity.room_owned {
                    ...
                    if room.state.is_done() {
                        {
                            if let Some(rc) =
                                entities.get_component_mut::<RoomController>(room.controller)
                            {
                ...
                if let Some(owned) = entity.owned {
                    entities.add_component(e, Owned { player_id: owned })
                }
                ...
                if let Some(tints) = entity.tints {
                    entities.add_component(e, Tints { tints });
                }
                    ...
                    if let Some(idx) = idx {
                        let rc = running_choices
                            ...
                            .or_insert_with(|| {
                                ...
                                crate::script_room::RunningChoice::new(
                                    log, engine, owned, name, idx,
                                )
                            });
                ...
                if let Some(money) = entity.money {
                    entities.add_component(e, Money { money: money.money });
                } else {
            ...
            SaveData::IdleScript(player_id, name, saved_state) => {
                ...
                if let Some(idx) = choices.student_idle.get_choice_index_by_name(name) {
                    let rc = running_choices
                        ...
                        .or_insert_with(|| {
                            let name =
                                assume!(log, choices.student_idle.get_choice_name_by_index(idx));
                            crate::script_room::RunningChoice::new(
                                log, engine, player_id, name, idx,
                            )
                        });
    ...
    init_players(
        sf_players,
        players,
        Some((&mut level, snapshots, entities)),
        &staff_list,
    );
...
impl SavableCourse {
    ...
    fn make_timetable_row(
        snapshots: &snapshot::Snapshots,
        v: [SavableCourseEntry; NUM_TIMETABLE_SLOTS],
    ) -> [course::CourseEntry; NUM_TIMETABLE_SLOTS] {
        let [v0, v1, v2, v3] = v;
        [
    ...
    fn make_timetable_entry(
        snapshots: &snapshot::Snapshots,
        v: SavableCourseEntry,
    ) -> course::CourseEntry {
        match v {
    ...
    fn conv_timetable(
        ids: &Read<NetworkId>,
        v: &[[course::CourseEntry; NUM_TIMETABLE_SLOTS]; 7],
    ) -> [[SavableCourseEntry; NUM_TIMETABLE_SLOTS]; 7] {
        [
    ...
    fn conv_timetable_day(
        ids: &Read<NetworkId>,
        v: &[course::CourseEntry; NUM_TIMETABLE_SLOTS],
    ) -> [SavableCourseEntry; NUM_TIMETABLE_SLOTS] {
        [
