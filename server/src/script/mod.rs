...
pub fn init_unilib(log: Logger, asset_manager: assets::AssetManager, lua: &lua::Lua) {
    ...
    lua.set(
        Scope::Global,
        "native_print",
        lua::closure1(move |_, msg: lua::Ref<String>| info!(log1, "{}", msg)),
    );
    ...
    lua.set(
        ...
        "get_module_script",
        lua::closure2(
            move |lua, m: lua::Ref<String>, ff: lua::Ref<String>| -> errors::Result<_> {
                let mut f = asset_manager.open_from_pack(assets::ModuleKey::new(&*m), &ff)?;
                let mut out = String::new();
                f.read_to_string(&mut out)?;

                let watched_files: lua::Ref<RefCell<WatchedFiles>> =
                    lua.get(Scope::Registry, WATCHED_FILES)?;
...
pub struct NulledString(#[doc(hidden)] pub &'static str);
...
pub fn handle_reloads<I: Invokable>(
    log: &Logger,
    engine: &I,
    ...
    let to_reload = {
        let watched_files: lua::Ref<RefCell<WatchedFiles>> =
            assume!(log, engine.get(Scope::Registry, WATCHED_FILES));
        let mut watched_files = watched_files.borrow_mut();
    ...
    for reload in to_reload {
        assume!(
            log,
            engine.invoke_function::<_, ()>("reload_module", lua::Ref::new_string(engine, reload))
        );
    }
}
...
impl Engine {
    ...
    pub fn new(log: &Logger, asset_manager: assets::AssetManager) -> Engine {
        ...
        assume!(
            ...
            engine
                .lua
                .execute_named_string::<()>("bootstrap", SCRIPT_BOOTSTRAP)
        );
        assume!(
            log,
            engine
                .lua
                .execute_named_string::<()>("server_bootstrap", SERVER_SCRIPT_BOOTSTRAP)
        );
    ...
    pub fn init_pack(&self, pack: &str) {
        if !assume!(
            self.log,
            self.invoke_function::<lua::Ref<String>, bool>(
                "load_module",
                lua::Ref::new_string(self, pack)
            )
        ) {
...
fn init_serialize(lua: &lua::Lua) {
    use delta_encode::bitio::{read_string, write_str};
    use lua::{Ref, Table};
    ...
    lua.set(
        Scope::Global,
        "serialize_create_desc",
        lua::closure1(|lua, desc: Ref<Table>| -> errors::Result<Ref<TableDesc>> {
            let mut tdesc = TableDesc { fields: vec![] };
    ...
    lua.set(
        ...
        lua::closure2(
            |lua,
             desc: Ref<TableDesc>,
             data: Ref<Table>|
             -> errors::Result<Ref<Arc<bitio::Writer<Vec<u8>>>>> {
