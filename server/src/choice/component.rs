...
impl<T> EntityVarStorage<T> {
    ...
    pub fn create<G>(alloc: BasicAlloc<G>, val: T) -> EntityVarStorage<T> {
        ...
        let mem = unsafe {
            alloc::alloc(
                alloc::Layout::from_size_align(var_size * 4 * 4, 4)
                    .expect("Failed to layout entity memory"),
            )
        };
        EntityVarStorage {
            val,
...
impl<T> Drop for EntityVarStorage<T> {
    fn drop(&mut self) {
        unsafe {
            alloc::dealloc(
                self.data as *mut _,
                alloc::Layout::from_size_align(self.var_size * self.size * 4, 4)
                    .expect("Failed to layout entity memory"),
            );
        }
    }
...
pub struct EntityVars<T>(
    pub *mut u32,
    &'static FNVMap<String, (Type, u16)>,
    PhantomData<T>,
);
...
impl<T> EntityVars<T> {
    ...
    pub fn iter<'a>(&'a self) -> impl Iterator<Item = (&'a str, u32)> + 'a {
        self.1.iter().map(move |v| {
            (v.0.as_str(), unsafe {
                self.0.offset((v.1).1 as isize).read()
            })
        })
    }
    ...
    pub fn slice(&self) -> &[u32] {
        unsafe { ::std::slice::from_raw_parts(self.0, self.1.len()) }
    }
    ...
    pub fn get_stat(&self, stat: Stat) -> f32 {
        unsafe {
            f32::from_bits(self.0.add(stat.index).read())
                .min(1.0)
                .max(0.0)
        }
    }
    ...
    fn add_component(&mut self, id: u32, _val: T) {
        if self.size <= id as usize {
            ...
            let mem = unsafe {
                alloc::realloc(
                    self.data as *mut _,
                    alloc::Layout::from_size_align(self.var_size * old_size * 4, 4)
                        .expect("Failed to layout entity memory"),
        ...
        unsafe {
            self.data
                .add(id as usize * self.var_size)
                .write_bytes(0, self.var_size);
        }
    }
