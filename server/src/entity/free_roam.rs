...
pub(crate) fn server_tick(
    ...
    tick::<crate::script_room::Types, _>(log, entities, scripting, player_info, "server_handler");
}
...
pub fn tick<E: script::ScriptTypes, C: 'static>(
    ...
    for e in entities.iter_mask(&mask).collect::<Vec<_>>() {
        let props = entities.with(
            |_em: EntityManager<'_>, mut roam_props: ecs::Write<LuaRoamEntityProperties>| {
        ...
        let script;
        {
            ...
            entities.with(
                |_em: EntityManager<'_>,
                 ...
                 object: ecs::Read<Object>| {
                    scope.insert(
                        Ref::new_string(scripting, "entity"),
                        E::from_entity(scripting, &mut entity_ref, &living, &object, e, None),
                    );
                },
            );
            ...
            scope.insert(
                Ref::new_string(scripting, "player"),
                i32::from(owned.player_id.0),
            );
            ...
            scope.insert(
                Ref::new_string(scripting, "notify_player"),
                lua::closure2(
                    move |lua,
                          de_func: Ref<String>,
        ...
        let coroutine = match scripting
            .with_borrows()
            .borrow_mut(entities)
            .borrow_mut(extra)
            .invoke_function::<_, Ref<Coroutine>>(
                "invoke_free_roam",
                (
        ...
        let props = assume!(
            log,
            entities.get_component_mut::<LuaRoamEntityProperties>(e)
        );
