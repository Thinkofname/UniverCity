        ...
        |_em: EntityManager,
         ...
         -> errors::Result<UniDollar> {
            ...
            'courses: for course in courses {
                ...
                timetable.add_component(e, TimeTable { course: course.uid });
...
pub(crate) fn book_into_course(
    log: &Logger,
    level_rooms: &LevelRooms,
    course: &course::Course,
    rc: &mut Write<RoomController>,
    e: ecs::Entity,
) {
    for (di, day) in course.timetable.iter().enumerate() {
        'day: for (pi, p) in day.iter().enumerate() {
            if let course::CourseEntry::Lesson { ref rooms, .. } = p {
                for lm in rooms {
                    let rm = level_rooms.get_room_info(lm.room);
...
closure_system!(
    pub(crate) fn manage_time_table(
        em: EntityManager<'_>,
        log: Read<CLogger>,
        ...
        for (e, (timetable, owned)) in em.group_mask((&mut timetable, &owned), |m| {
            m.and(&student)
                .and_not(&frozen)
                .and_not(&goto_room)
