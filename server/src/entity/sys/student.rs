...
closure_system!(
    pub fn get_timetable(
        ...
        for (e, (pos, owned)) in em.group_mask((&pos, &owned), |m| {
            m.and(&student)
                .and_not(&frozen)
                .and_not(&goto_room)
                .and_not(&quitting)
            ...
            if timetable.get_component(e).is_some()
                && timetable_completed.get_component(e).is_none()
            {
                continue;
            }
...
closure_system!(
    pub(crate) fn quit_sys(
        ...
        for (e, position) in em.group_mask(&position, |m| m.and(&quit)) {
            if path_info.get_component(e).is_some() || target.get_component(e).is_some() {
                continue;
            }

            // Clear the student's timetable to free up space in rooms
            if let Some(t) = timetable.remove_component(e) {
                super::timetable::clear_time_table(
                    &log.log,
                    e,
                    rooms,
                    t,
                    ...
                    for (pi, p) in d.iter().enumerate() {
                        if let Some(course) = *p {
                            if let Some(course) = player.courses.get_mut(&course) {
                                if let course::CourseEntry::Lesson { ref mut rooms, .. } =
                                    course.timetable[di][pi]
                                {
                                    for r in rooms {
                                        if r.staff == e {
