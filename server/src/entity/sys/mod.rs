...
closure_system!(
    pub fn open_door_server(em: EntityManager<'_>, mut door: Write<Door>) {
        use std::cmp::max;
...
closure_system!(
    pub fn lifetime_sys(em: EntityManager<'_>, mut lifetime: Write<Lifetime>) {
...
closure_system!(
    pub fn tick_emotes(em: EntityManager<'_>, mut emotes: Write<IconEmote>) {
...
closure_system!(
    pub(super) fn pay_staff(
        ...
        for (e, (paid, owned)) in em.group_mask((&mut paid, &owned), |m| m.and_not(&frozen)) {
            if paid
                .last_payment
                .map_or(false, |v| day.day.wrapping_sub(v) < 3)
            {
                continue;
            }
...
closure_system!(
    pub(crate) fn tick_student_stats(
        em: EntityManager<'_>,
        ...
        for (e, _owned) in em.group_mask(&owned, |m| m.and_not(&frozen).and(&vars)) {
            ...
            vars.set_stat(
                Stats::STUDENT_HAPPINESS,
                happiness - (1.0 / (20.0 * 60.0 * 6.0)) * (1.5 - hunger),
            );
            vars.set_stat(Stats::STUDENT_HUNGER, hunger - 1.0 / (20.0 * 60.0 * 4.0));
        }
    }
);

closure_system!(
    pub(crate) fn tick_professor_stats(
        em: EntityManager<'_>,
        ...
        for (e, owned) in em.group_mask(&owned, |m| m.and_not(&frozen).and(&vars)) {
            let vars = assume!(log.log, vars.get_custom(e));
            ...
            vars.set_stat(
                Stats::PROFESSOR_FATIGUE,
                fatigue - 1.0 / (20.0 * 60.0 * 12.0),
            );
            ...
            let happiness = vars.get_stat(Stats::PROFESSOR_HAPPINESS);
            vars.set_stat(
                Stats::PROFESSOR_JOB_SATISFACTION,
                job_satisfaction
                    - (1.0 / (20.0 * 60.0 * 60.0))
...
closure_system!(
    pub(crate) fn rest_staff(
        ...
        for (e, (owned, pos)) in em.group_mask((&owned, &pos), |m| {
            m.and(&auto_rest)
                .and_not(&room_owned)
                .and_not(&frozen)
                .and_not(&goto_room)
            ...
            let nearest_sr = rooms
                ...
                .filter(|v| {
                    ...
                    rc.visitors.len() + rc.waiting_list.len() + rc.potential_list.len()
                        < rc.capacity
                })
                .min_by(|a, b| {
