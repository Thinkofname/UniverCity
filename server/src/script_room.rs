        ...
        let owned_entities =
            rc.iter()
                ...
                .fold(Ref::new_table(lua), |tbl, e| {
                    tbl.insert(
                        tbl.length() + 1,
                        LuaEntityRef::get_or_create(
                            entity_ref,
                            living,
                            object,
                            lua,
                            *e,
                            Some(Controller::Room(room.id)),
                        ),
                    );
                    tbl
                });
...
impl lua::LuaUsable for LuaRoom {
    ...
    fn fields(t: &lua::TypeBuilder) {
        ...
        t.field(
            "get_room_id",
            lua::closure1(|_lua, this: Ref<LuaRoom>| i32::from(this.id.0)),
        );
        // Returns whether the room is closing
        t.field(
            "get_closing",
            lua::closure1(|_lua, this: Ref<LuaRoom>| this.closing),
        );
        // Returns the size of the room in tiles.
        t.field(
            "get_size",
            lua::closure1(|_lua, this: Ref<LuaRoom>| (this.width, this.height)),
        );
        t.field(
            "get_bounds",
            lua::closure1(|lua, this: Ref<LuaRoom>| -> UResult<_> {
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
                let rooms = rooms.borrow();
                let room = rooms
                    .try_room_info(this.id)
        ...
        t.field(
            ...
            lua::closure1(|lua, this: Ref<LuaRoom>| -> UResult<_> {
                ...
                let room = rooms
                    .try_room_info(this.id)
                    .ok_or(ErrorKind::StaleScriptReference)?;
                Ok((room.area.min.x, room.area.min.y))
            }),
        );
        t.field(
            "get_valid",
        ...
        t.field(
            ...
            lua::closure1(|lua, this: Ref<LuaRoom>| -> UResult<_> {
                ...
                Ok(ty
                    .controller
                    .as_ref()
                    .map(|v| Ref::new_string(lua, v.as_string())))
            }),
        );
        t.field(
            "get_idle_task",
            lua::closure2(
                |lua, this: Ref<LuaRoom>, name: Ref<String>| -> UResult<Option<_>> {
                    let rooms = lua
                        .get_tracked::<LevelRooms>()
                        .ok_or_else(|| ErrorKind::InvalidState)?;
                    let rooms = rooms.borrow();
                    let room = rooms
                        ...
                        .ok_or(ErrorKind::StaleScriptReference)?;
                    Ok(unsafe {
                        lua.get_unsafe::<lua::Ref<Table>>(lua::Scope::Global, b"idle_storage\0")
                    }
                    .ok()
        ...
        t.field(
            "get_properties",
            lua::closure1(|_lua, this: Ref<LuaRoom>| this.properties.clone()),
        );
            ...
            lua::closure1(|lua, this: Ref<LuaRoom>| -> UResult<_> {
                ...
                Ok(room
                    ...
                    .and_then(|v| {
                        ...
                        lua::with_table_serializer(lua, |se| {
        ...
        t.field(
            "set_sync_state",
            lua::closure2(|lua, this: Ref<LuaRoom>, state: Ref<Table>| -> UResult<_> {
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
                let mut rooms = rooms.borrow_mut();
                let room = rooms
                    .try_room_info_mut(this.id)
                    .ok_or(ErrorKind::StaleScriptReference)?;
        ...
        t.field(
            ...
            lua::closure3(
                |lua,
                 this: Ref<LuaRoom>,
                 method: Ref<String>,
                 data: Ref<Arc<bitio::Writer<Vec<u8>>>>|
                 -> UResult<_> {
                    let commands = lua
                        .get_tracked::<ExtraCommands>()
                        .ok_or_else(|| ErrorKind::InvalidState)?;
                    let mut commands = commands.borrow_mut();
                    commands.push(
                        command::ExecRoom::new(
                            this.id,
                            method.to_string(),
                            common::ScriptData(Arc::clone(&data)),
                        )
                        .into(),
                    );
                    Ok(())
        ...
        t.field(
            "request_entity",
            lua::closure2(|lua, this: Ref<LuaRoom>, ty: Ref<String>| -> UResult<_> {
                ...
                let ty = assets::LazyResourceKey::parse(&ty).or_module(controller.module_key());

                Ok(
                    if let Some(entity) = request_entity_now(
                        &log,
                        room.owner,
                        ty.borrow(),
                        Some(pos),
                        Controller::Room(room.id),
                        &mut *entities,
                    ) {
                        {
                            let rc = entities
                        ...
                        entities
                            .add_component(entity, Controlled::new_by(Controller::Room(room.id)));
                        entities.with(
                            |_em: EntityManager<'_>,
                             mut entity_ref: ecs::Write<LuaEntityRef>,
                             living: ecs::Read<Living>,
                             object: ecs::Read<Object>| {
                                Some(LuaEntityRef::get_or_create(
                                    &mut entity_ref,
                                    &living,
                                    &object,
                                    lua,
        ...
        t.field(
            "get_owned",
            lua::closure1(|_lua, this: Ref<LuaRoom>| this.owned_entities.clone()),
        );
        // Returns a list of entities that are visiting the room
        // (e.g. students).
        t.field(
            "get_visitors",
            lua::closure1(|_lua, this: Ref<LuaRoom>| this.visitors.clone()),
        );
        t.field(
            "get_objects",
            lua::closure1(|lua, this: Ref<LuaRoom>| -> UResult<_> {
        ...
        t.field(
            "object_index",
            lua::closure2(
                |_lua, _this: Ref<LuaRoom>, o: Option<Ref<LuaObject<Types>>>| {
        ...
        t.field(
            "set_capacity",
            lua::closure2(|lua, this: Ref<LuaRoom>, val: i32| -> UResult<_> {
                let mut entities = lua.write_borrow::<Container>();
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
        ...
        t.field(
            "get_capacity",
            lua::closure1(|lua, this: Ref<LuaRoom>| -> UResult<i32> {
                ...
                let room = rooms
                    ...
                    .ok_or(ErrorKind::StaleScriptReference)?;
                let rc = entities
                    .get_component::<RoomController>(room.controller)
                    .ok_or(ErrorKind::StaleScriptReference)?;
        ...
        t.field(
            "get_has_waiting",
            lua::closure1(|lua, this: Ref<LuaRoom>| -> UResult<bool> {
                let entities = lua.read_borrow::<Container>();
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
        ...
        t.field(
            "owns_tile",
            lua::closure3(|lua, this: Ref<LuaRoom>, x: i32, y: i32| -> UResult<bool> {
                let tiles = lua
        ...
        t.field(
            ...
            lua::closure3(
                move |lua,
                    ...
                    let (script, method) =
                        if let Some(pos) = de_func.char_indices().find(|v| v.1 == '#') {
                    ...
                    player
                        .notifications
                        .push(crate::notify::Notification::Script {
                            script,
                            func,
    ...
    T: script::ScriptTypes + 'static,
{
    ...
    fn fields(t: &lua::TypeBuilder) {
        t.field(
            "get_id",
            lua::closure1(|_lua, this: Ref<LuaObject<T>>| this.object as i32 + 1),
        );
        t.field(
            "get_valid",
            lua::closure1(|lua, this: Ref<LuaObject<T>>| -> UResult<_> {
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
                let rooms = rooms.borrow();
                Ok(rooms
                    .try_room_info(this.room)
                    .and_then(|room| room.objects.get(this.object).and_then(|v| v.as_ref()))
                    .map_or(false, |_| true))
            }),
        );
        t.field(
        ...
        t.field(
            "get_room_id",
            lua::closure1(|_lua, this: Ref<LuaObject<T>>| -> UResult<_> {
        ...
        t.field(
            ...
            lua::closure1(|lua, this: Ref<LuaObject<T>>| -> UResult<_> {
                ...
                let room = rooms
                    ...
                    .ok_or(ErrorKind::StaleScriptReference)?;
                let props = entities
                    .get_component_mut::<LuaRoomProperties>(room.controller)
                    .ok_or(ErrorKind::StaleScriptReference)?;
                    ...
                    .filter_map(|v| {
                        ...
                        match *v {
                            ObjectPlacementAction::WallFlag {
                                ...
                                out.insert(
                                    Ref::new_string(lua, "type"),
                                    Ref::new_string(lua, "wall"),
                                );
                                ...
                                out.insert(
                                    Ref::new_string(lua, "direction"),
                                    Ref::new_string(lua, direction.as_str()),
                                );
                                out.insert(
                                    Ref::new_string(lua, "flag"),
                                    Ref::new_string(
                                        lua,
                                        match *flag {
                            ...
                            ObjectPlacementAction::Tile {
                                ...
                                out.insert(
                                    Ref::new_string(lua, "tile"),
                                    Ref::new_string(lua, key.as_string()),
                                );
                            }
                            _ => return None,
                        }
            ...
            lua: Ref::new(
                ...
                LuaEntity {
                    ...
                    key: key.unwrap_or_else(|| {
                        if let Some(living) = living.get_component(entity) {
...
impl lua::LuaUsable for LuaEntity {
    ...
    fn fields(t: &lua::TypeBuilder) {
        t.field(
            "get_valid",
            lua::closure1(|lua, this: Ref<LuaEntity>| {
                let entities = lua.read_borrow::<Container>();
                ...
                if let Some(props) = entities.get_component::<LuaEntityRef>(this.entity) {
                    ...
                    props.controller
                        == entities
                            .get_component::<Controlled>(this.entity)
                            .and_then(|v| v.by)
                } else {
                    false
                }
        ...
        t.field(
            ...
            lua::closure1(|lua, this: Ref<LuaEntity>| {
                let entities = lua.read_borrow::<Container>();
                entities
                    .get_component::<pathfind::PathInfo>(this.entity)
                    .is_some()
                    || entities
                        .get_component::<pathfind::Target>(this.entity)
                        .map_or(false, |v| v.request.is_some())
            }),
        );
        ...
        t.field(
            ...
            lua::closure1(|lua, this: Ref<LuaEntity>| {
                let entities = lua.read_borrow::<Container>();
                entities
                    .get_component::<NetworkId>(this.entity)
                    .map(|v| v.0 as i32)
            }),
        );
        ...
        t.field(
            "get_in_room",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> UResult<_> {
                let entities = lua.read_borrow::<Container>();
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
        ...
        t.field(
            "walk_to",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, mut x: f64, mut y: f64| -> UResult<()> {
                    let mut entities = lua.write_borrow::<Container>();
                    let rooms = lua
                        .get_tracked::<LevelRooms>()
                        .ok_or_else(|| ErrorKind::InvalidState)?;
                    let rooms = rooms.borrow();
                    let bound = if let Some(Controller::Room(room_id)) = entities
                        .get_component::<Controlled>(this.entity)
                        .and_then(|v| v.by)
                    {
                        let room = rooms.get_room_info(room_id);
                    ...
                    if !bound.in_bounds(loc) {
                        bail!(
                            "Can't path outside room {:#?} not in {:#?}. {:?}",
                            loc,
                            bound,
                    ...
                    entities
                        .add_component(this.entity, pathfind::Target::try_new(x as f32, y as f32));
                    Ok(())
                },
            ),
        );
        ...
        t.field(
            "walk_to_face",
            lua::closure4(
                |lua, this: Ref<LuaEntity>, mut x: f64, mut y: f64, facing: f64| -> UResult<()> {
                    let mut entities = lua.write_borrow::<Container>();
                    let rooms = lua
                        .get_tracked::<LevelRooms>()
                        .ok_or_else(|| ErrorKind::InvalidState)?;
        ...
        t.field(
            "get_key",
            lua::closure1(|_lua, this: Ref<LuaEntity>| -> Ref<String> { this.key.clone() }),
        );
        // Returns the position of this entity
        t.field(
            "get_position",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> UResult<(f64, f64)> {
                let entities = lua.read_borrow::<Container>();
                let rooms = lua
                    .get_tracked::<LevelRooms>()
                    .ok_or_else(|| ErrorKind::InvalidState)?;
                ...
                if let Some(pos) = entities.get_component::<Position>(this.entity) {
                    Ok((
                        f64::from(pos.x) - f64::from(bound.min.x),
                        f64::from(pos.z) - f64::from(bound.min.y),
                    ))
                } else {
                    bail!("Invalid entity")
                }
        ...
        t.field(
            "get_rotation",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> Option<f64> {
                let entities = lua.read_borrow::<Container>();
        ...
        t.field(
            "get_size",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> Option<(f64, f64)> {
                let entities = lua.read_borrow::<Container>();
        ...
        t.field(
            "get_properties",
            lua::closure1(|_lua, this: Ref<LuaEntity>| this.props.clone()),
        );
            ...
            lua::closure1(|lua, this: Ref<LuaEntity>| {
                ...
                if let Some(state) = entities.get_component::<StateData>(this.entity) {
                    if state.data.bit_len() == 0
                        || (state.controller != c && state.controller.is_some())
                    {
                        None
                    } else {
        ...
        t.field(
            "generate_time_table",
            lua::closure1(
                |lua, this: Ref<LuaEntity>| -> UResult<(i32, Option<Ref<String>>)> {
                    let mut entities = lua.write_borrow::<Container>();
        ...
        t.field(
            "get_should_release",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> bool {
                let entities = lua.read_borrow::<Container>();
                entities
                    .get_component::<Controlled>(this.entity)
        ...
        t.field(
            "set_should_release",
            lua::closure2(|lua, this: Ref<LuaEntity>, should_release: bool| {
                let mut entities = lua.write_borrow::<Container>();
                entities
                    .get_component_mut::<Controlled>(this.entity)
        ...
        t.field(
            "get_should_release_inactive",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> bool {
                let entities = lua.read_borrow::<Container>();
                entities
                    .get_component::<RoomOwned>(this.entity)
        ...
        t.field(
            "get_active",
            lua::closure1(|lua, this: Ref<LuaEntity>| -> UResult<bool> {
                let entities = lua.read_borrow::<Container>();
                let owner = entities
                    .get_component::<RoomOwned>(this.entity)
                    .ok_or(ErrorKind::StaleScriptReference)?;
        ...
        t.field(
            "set_active",
            lua::closure2(|lua, this: Ref<LuaEntity>, val: bool| -> UResult<_> {
                let mut entities = lua.write_borrow::<Container>();
                let owner = entities
                    .get_component_mut::<RoomOwned>(this.entity)
                    .ok_or(ErrorKind::StaleScriptReference)?;
                owner.active = val;
                Ok(())
            }),
        ...
        t.field(
            "charge",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, service: Ref<String>, money: i32| -> UResult<_> {
                    let mut entities = lua.write_borrow::<Container>();
                    let log = lua
                        .get_tracked::<Logger>()
                        .ok_or_else(|| ErrorKind::InvalidState)?;
        ...
        t.field(
            "change_stat",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, name: Ref<String>, modi: f64| -> UResult<_> {
                    let mut entities = lua.write_borrow::<Container>();
        ...
        t.field(
            "get_var_float",
            lua::closure2(
                |lua, this: Ref<LuaEntity>, name: Ref<String>| -> UResult<_> {
                    let mut entities = lua.write_borrow::<Container>();
        ...
        t.field(
            "set_var_float",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, name: Ref<String>, val: f64| -> UResult<_> {
                    let mut entities = lua.write_borrow::<Container>();
                    let vars = get_vars(&mut *entities, this.entity);
                    if let Some(vars) = vars {
        ...
        t.field(
            "set_var_int",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, name: Ref<String>, val: i32| -> UResult<_> {
                    let mut entities = lua.write_borrow::<Container>();
                    let vars = get_vars(&mut *entities, this.entity);
                    if let Some(vars) = vars {
        ...
        t.field(
            "set_var_bool",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, name: Ref<String>, val: bool| -> UResult<_> {
                    let mut entities = lua.write_borrow::<Container>();
                    let vars = get_vars(&mut *entities, this.entity);
                    if let Some(vars) = vars {
        ...
        t.field(
            "give_money",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, reason: Ref<String>, money: i32| -> UResult<_> {
        ...
        t.field(
            "give_rating",
            lua::closure3(
                |lua, this: Ref<LuaEntity>, reason: Ref<String>, rating: i32| -> UResult<_> {
                    use std::cmp;
                    let entities = lua.read_borrow::<Container>();
                    let log = lua
                        .get_tracked::<Logger>()
        ...
        t.field(
            "give_grade",
            lua::closure2(
                |lua, this: Ref<LuaEntity>, grade: Ref<String>| -> UResult<bool> {
                    let mut entities = lua.write_borrow::<Container>();
                    let log = lua
                        .get_tracked::<Logger>()
                        .ok_or_else(|| ErrorKind::InvalidState)?;
                    ...
                    if let Some((day, slot)) = entities
                        .get_component::<Activity>(this.entity)
                        .map(|v| (v.day, v.slot))
                    {
...
impl RunningChoice {
    pub(crate) fn new(
        ...
        let map = assume!(
            log,
            scripting.get::<lua::Ref<Table>>(lua::Scope::Global, "idle_storage")
        );
        ...
        player_map.insert(
            Ref::new_string(scripting, name.as_string()),
            handle.props.clone(),
        );
        RunningChoice {
            _player: player,
...
impl lua::LuaUsable for IdleScriptHandle {
    ...
    fn fields(t: &lua::TypeBuilder) {
        ...
        t.field(
            "get_rooms",
            lua::closure1(|lua, this: Ref<IdleScriptHandle>| {
        ...
        t.field(
            "get_properties",
            lua::closure1(|_lua, this: Ref<IdleScriptHandle>| this.props.clone()),
        );
        // // Returns the room's id
        t.field(
            "get_room_by_id",
            lua::closure2(|lua, this: Ref<IdleScriptHandle>, id: i32| -> UResult<_> {
                ...
                entities.with(
                    |_em: EntityManager<'_>,
                     ...
                     object: ecs::Read<Object>| {
                        if rooms
                            .try_room_info(id)
                            .map_or(false, |v| v.owner == this.player)
                        {
                            Ok(Types::from_room(
                                &log,
                                lua,
    ...
    for ((player, idx), ref mut rc) in &mut running_choices.choices {
        ...
        match scripting
            ...
            Ok(val) => {
                ...
                assume!(
                    log,
                    with_table_deserializer(&val, |de| { serde_transcode::transcode(de, &mut se) })
                );
                let data = se.into_inner();
            ...
            Err(err) => {
                error!(log, "Failed to sync idle script"; "script" => ?script, "error" => %err)
            }
        }
    }
    ...
    entities.with(
        |em: EntityManager<'_>,
         ...
         mut controlled: Write<Controlled>| {
            ...
            for (e, (idle, _living, owned, c)) in em
                .group_mask((&mut idle, &living, &owned, &mut controlled), |m| {
                    m.and(&vars)
                })
            {
                ...
                if idle.released {
                    ...
                    if c.wanted
                        .map_or(false, |v| v > Controller::Idle(0) && !v.is_idle())
                    {
                        c.by = None;
                        c.should_release = false;
                        to_remove.push(e);
                ...
                if idle.current_choice.is_none() {
                    ...
                    let selected = choices
                        .student_idle
                        .choose(&mut rng, &choices.global.wrap(&vars));
                    if let Some((idx, _selected)) = selected {
                        let rc = running_choices
                            .choices
                            .entry((owned.player_id, idx))
                            .or_insert_with(|| {
                                let name = assume!(
                                    log,
                                    choices.student_idle.get_choice_name_by_index(idx)
                                );
                    ...
                    idle.current_choice
                        .map(|v| c.by = Some(Controller::Idle(v)));
                ...
                |_em: EntityManager<'_>,
                 ...
                 mut entity_ref: Write<LuaEntityRef>| {
                    LuaEntityRef::get_or_create(
                        ...
                        scripting,
                        e,
                        Some(Controller::Idle(*idx)),
                    )
                },
            );
            if let Err(err) = scripting
                .with_borrows()
                .borrow_mut(entities)
                .borrow_mut(players)
                .invoke_function::<_, ()>(
    ...
    for room in level.room_ids() {
        let (needs_update, ty) = {
            ...
            (
                ...
                assume!(
                    log,
                    asset_manager.loader_open::<room::Loader>(room.key.borrow())
                ),
            )
        };
        if let Some(controller) = ty.controller.as_ref() {
            let lua_room =
                LuaRoom::from_room(log, &*level.rooms.borrow(), entities, room, scripting);
            if needs_update {
                if let Err(err) = scripting
                    .with_borrows()
                    .borrow_mut(entities)
...
impl script::ScriptTypes for Types {
    ...
    fn from_room(
        ...
        LuaRoom::from_room_raw(
            log, lua, rooms, props, rc, entity_ref, living, object, room_id,
        )
    }
}
...
impl<'a> CommandHandler for Handler<'a> {
    ...
    where
        E: Invokable,
    {
        let rc = if let Some(rc) = self
            .running_choices
            .choices
            .get(&(cmd.player, cmd.idx as usize))
        {
            rc
        } else {
        ...
        let script = assume!(
            ...
            self.choices
                .student_idle
                .get_choice_by_index(cmd.idx as usize)
        );
